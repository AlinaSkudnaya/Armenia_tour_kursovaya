...
$mail->Subject = 'Заявка с сайта на тур Ереван';
$mail->Body    = '' .$name . ' оставил(-a) заявку, его(eё) почта : ' .$email. '<br>Сообщение пользователя: ' .$message;
$mail->AltBody = '';

if(!$mail->send()) {
    echo 'Error';
} else {
    header('location:  thank-you_tour.html');
}
?>